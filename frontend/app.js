...
document.querySelector('#queue-form').addEventListener('submit', async (event) => {
  ...
  result.hidden = false;
  result.innerHTML = response.ok ? `<strong>Your token is #${data.token_number}</strong><br>We have added you to the ${escapeHtml(data.service_name)} queue. Keep this token to check your status.` : `<strong>Could not join the queue.</strong><br>${escapeHtml(data.detail || 'Please try again.')}`;
});
