...
let token = localStorage.getItem('salon_owner_token');
...
function renderQueue(entries) {
  const waiting = entries.filter(x => x.status === 'waiting');
  const serving = entries.find(x => x.status === 'serving');
  $('#waiting-count').textContent = waiting.length;
  $('#serving-token').textContent = serving ? `#${serving.token_number}` : '—';
...
async function loadDashboard() { try { const [queue, gallery] = await Promise.all([api('/api/admin/queue'), fetch(`${API}/api/gallery`).then(r => r.json())]); renderQueue(queue); renderGallery(gallery); } catch (e) { showToast(e.message, true); } }
function renderGallery(items) { $('#gallery-admin').innerHTML = items.length ? items.map(x => `<article class="gallery-item"><img src="${escapeAttr(x.image_url)}" alt="${escapeAttr(x.title)}"><h3>${escapeHtml(x.title)}</h3><button class="small-button danger-button" onclick="removeGallery(${x.id})">Remove</button></article>`).join('') : '<p class="muted">No gallery images yet.</p>'; }
async function complete(id) { try { await api(`/api/admin/queue/${id}/complete`, { method: 'POST' }); showToast('Service completed'); loadDashboard(); } catch (e) { showToast(e.message, true); } }
...
$('#login-form').addEventListener('submit', async (e) => { e.preventDefault(); $('#login-error').hidden = true; try { const data = await fetch(`${API}/api/auth/login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username: $('#username').value, password: $('#password').value }) }).then(async r => { const d = await r.json(); if (!r.ok) throw new Error(d.detail || 'Login failed'); return d; }); token = data.access_token; localStorage.setItem('salon_owner_token', token); $('#login-view').hidden = true; $('#dashboard-view').hidden = false; loadDashboard(); } catch (e) { $('#login-error').textContent = e.message; $('#login-error').hidden = false; } });
...
$('#service-form').addEventListener('submit', async (e) => { e.preventDefault(); try { await api('/api/admin/services', { method: 'POST', body: JSON.stringify({ name: $('#service-name').value, duration_minutes: Number($('#service-duration').value), description: $('#service-description').value || null }) }); e.target.reset(); $('#service-duration').value = 30; showToast('Service added'); } catch (e) { showToast(e.message, true); } });
$('#gallery-form').addEventListener('submit', async (e) => { e.preventDefault(); try { await api('/api/admin/gallery', { method: 'POST', body: JSON.stringify({ title: $('#gallery-title').value, image_url: $('#gallery-url').value, description: $('#gallery-description').value || null }) }); e.target.reset(); showToast('Gallery image added'); loadDashboard(); } catch (e) { showToast(e.message, true); } });
if (token) { $('#login-view').hidden = true; $('#dashboard-view').hidden = false; loadDashboard(); }
